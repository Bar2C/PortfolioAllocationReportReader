...
def process_pdf_tables(pdf_path):
    ...
    tables = tabula.read_pdf(pdf_path, guess=True, pages='all', pandas_options={'header': None}, area=area_coordinates)
    ...
    for df in tables:
        ...
        if df.shape[1] >= 2:
            ...
            for index, row in df.iterrows():
                ...
                last_added = last_column_value

                # Check if the first column value is in the lines list
                if first_column_value in lines :
                    ...
                    if first_column_value in result_dict:
                        if last_added == first_column_value:
